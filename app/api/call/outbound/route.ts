...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!baseUrl || baseUrl.includes('localhost')) {
      console.warn('[Call/Outbound] NEXT_PUBLIC_APP_URL is localhost â€” Twilio webhooks will fail. Run ngrok and set the public URL.')
    }
    const client = twilio(accountSid, authToken)
    const twimlUrl = new URL(`${baseUrl}/api/call/twiml`)
    if (message) twimlUrl.searchParams.set('reason', message)
    ...
    console.log('[Call/Outbound] created â€” sid:', call.sid, '| status:', call.status)
    if (userId) {
    ...
    console.error('[Call/Outbound] Twilio error â€” code:', e.code, '| message:', e.message, '| moreInfo:', e.moreInfo)
